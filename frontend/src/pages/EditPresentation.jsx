...
function EditPresentation ({ token }) {
  ...
  // const [refetchData, setRefechData] = React.useState(false);
  ...
  // const handleThumbnailUpdate = async (presentationId) => {
  //   const newThumbnail = prompt('Enter the URL for the new thumbnail:');
  //   if (newThumbnail) {
  //     try {
  //       await axios.put(`${BACKEND_URL}/presentation/${presentationId}/thumbnail`, { thumbnail: newThumbnail }, {
  //         headers: {
  //           Authorization: token,
  //         },
  //       });
  ...
  //   try {
  //     const updatedSlide = slides[currentSlideIndex].content.push(newSlideElement);
  ...
  React.useEffect(() => {
    if (Object.keys(newSlideElement).length !== 0) {
      // Assuming `slides` is an object with a property `content` that is an array
  ...
  // Store all changes in the backend
  ...
  // Makes the sidebar buttons grey when hovered
  const [hoveredButton, setHoveredButton] = React.useState(null);
  const hoverStyle = { color: '#D3D3D3' };

  // Makes the Title grey and boxed when hovered
